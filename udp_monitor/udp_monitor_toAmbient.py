...
# Ambientへの送信用設定
ambient_device = 'humid_3'          # Ambientへ送信するUDP送信デバイス名を入力
ambient_chid='00000'                # ここにAmbientで取得したチャネルIDを入力
ambient_wkey='0123456789abcdef'     # ここにはライトキーを入力
ambient_board_id='00000'            # ここにはAmbientのボードIDを入力(省略可)
ambient_interval = 30               # Ambientへの送信間隔
time_toAmbient = 0                  # Ambient 送信時刻をセット
...
import urllib.request                               # HTTPリクエストAmbient用
                ...
                if sort_col == 'devices':
                    ...
                    if i == 0:
                        ...
                        if hist > 0:
                            ...
                            html += dev[0:7] + '<br>[<a href="' + filename + '">csv</a>]'
                            if ambient_board_id != '00000' and dev[0:7] == ambient_device:
                                html += '[<a href="https://ambidata.io/bd/board.html?id=' + ambient_board_id + '">Ambient</a>]'
                            html += '</td>'
...
while True:
    ...
    # 数値データの変数保持(HTML表示用)
    body_dict = {'writeKey':ambient_wkey}               # Ambient送信用
    if dev[0:5] in sensors:                             # センサ(数値データ)のとき
        ...
        # print(dev_vals)
        if ambient_device == dev[0:7]:
            d = 1
            for val in vals:
                key = 'd' + str(d)
                body_dict[key] = str(get_val(val))
                # valsはUDP生値につき危険かもしれないので念のため数値変換
                d += 1
                if d > 8:
                    break

    # Ambientへの送信処理
    if int(ambient_chid) != 0 and len(body_dict) > 1  and time.time() >= time_toAmbient:
        ...
        if len(res_str):                                    # 受信テキストがあれば
            ...
            print('Done')                                   # Doneを表示
        time_toAmbient = time.time() + ambient_interval     # Ambient 送信時刻をセット
sock.close()                                            # ソケットの切断
